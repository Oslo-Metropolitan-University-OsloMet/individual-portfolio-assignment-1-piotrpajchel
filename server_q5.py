...
server.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)  # Make serveradress reusable
...
def q():  # Function for sending a message from one client to all active clients
...
def handel(client):  # Function for handling clients if client not available remove client from server
    while True:
        ...
        if message:  # if message is not zero byte
            broadcast(message)
...
def main():
    ...
    thread_q = threading.Thread(target=q)
    thread_q.start()
